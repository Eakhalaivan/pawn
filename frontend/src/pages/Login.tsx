  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        setSuccess('Account created! Please check your email for a verification link to complete sign up.');
        setIsSignUp(false);
      } else {
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <div className="mt-6">
            <button
              onClick={switchMode}
              disabled={loading}
              className="w-full text-center text-sm text-purple-600 hover:text-purple-500 disabled:opacity-75 disabled:cursor-not-allowed"
            >
              {isSignUp
